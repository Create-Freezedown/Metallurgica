...
public abstract class MetallurgicaRegistry<K, V> implements Iterable<V> {

    public static final Map<ResourceLocation, MetallurgicaRegistry<?, ?>> REGISTERED = new HashMap<>();
    @Getter
    ...
    public MetallurgicaRegistry(ResourceLocation registryName) {
    ...
    public void freeze() {
        ...
        if (!checkActiveModContainerIsMetallurgica()) {
            return;
        }
    ...
    private boolean checkActiveModContainerIsMetallurgica() {
        ModContainer container = ModLoadingContext.get().getActiveContainer();
        return container != null && (container.getModId().equals(this.registryName.getNamespace()) ||
                container.getModId().equals(Metallurgica.ID) ||
    ...
    public <T extends V> T replace(K key, T value) {
        ...
        if (!containKey(key)) {
            Metallurgica.LOGGER.warn("[replace] couldn't find key %s in registry %s".formatted(registryName, key));
        }
        registry.put(key, value);
        return value;
    }
    ...
    public static class String<V> extends MetallurgicaRegistry<java.lang.String, V> {
    ...
    public static class RL<V> extends MetallurgicaRegistry<ResourceLocation, V> {
