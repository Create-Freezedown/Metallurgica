...
public class ShakingTableBlockMethods {
    
    protected static ShakingTableBehaviour get(BlockGetter worldIn, BlockPos pos) {
        return BlockEntityBehaviour.get(worldIn, pos, ShakingTableBehaviour.TYPE);
    }
    ...
    public static InteractionResult onUse(BlockState state, Level world, BlockPos pos, Player player,
        ...
        ShakingTableBehaviour behaviour = get(world, pos);
        if (behaviour == null)
            return InteractionResult.PASS;
    ...
    public static int getComparatorInputOverride(BlockState blockState, Level worldIn, BlockPos pos) {
        ShakingTableBehaviour shakingTableBehaviour = get(worldIn, pos);
        if (shakingTableBehaviour == null)
            return 0;
        float f = shakingTableBehaviour.getPresentStackSize();
        Integer max = shakingTableBehaviour.maxStackSize.get();
        f = f / (max == 0 ? 64 : max);
